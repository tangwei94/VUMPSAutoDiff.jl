...
# NOTE. the DIIS extrapolation doesn't work well in the case of VOMPS. (for a demo, see test/test_vomps/test_diis_vomps.jl)
# I just keep it here for the record.
...
function mpo_power_iterations(T::MPOTensor, AL::MPSTensor, AR::MPSTensor, AC::MPSTensor, C::MPSBondTensor, options::MPOPowerIterationOptions)
    ...
    if options.max_diis_step > 0
        B = initialize_ovlpmat(subspace_errs; damping_factor=options.damping_factor, inner=TensorKit.inner)
