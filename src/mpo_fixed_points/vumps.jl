...
function project_dAL!(out::TensorMap, dAL::TensorMap, AL::AbstractTensorMap, manifold::Symbol)
    copy!(out, dAL)
    if manifold == :Grassmann
        TensorKitManifolds.Grassmann.project!(out, AL)
        return out
    elseif manifold == :Stiefel
        TensorKitManifolds.Stiefel.project!(out, AL)
        return out
    else
        error("manifold not supported")
    end
end
project_dAL!(::Any, dAL::Union{ZeroTangent, NoTangent}, ::AbstractTensorMap, ::Symbol) = dAL
...
function project_dAR!(out::TensorMap, dAR::TensorMap, AR::AbstractTensorMap, manifold::Symbol)
    copy!(out, dAR)
    if manifold == :Grassmann
        AR_adjoint = permute(AR, ((1, ), (2, 3)))'
        dAR_adjoint = permute(out, ((1, ), (2, 3)))'
        TensorKitManifolds.Grassmann.project!(dAR_adjoint, AR_adjoint)
        permute!(out, dAR_adjoint', ((1, 2), (3, )))
        return out
    elseif manifold == :Stiefel
        AR_adjoint = permute(AR, ((1, ), (2, 3)))'
        dAR_adjoint = permute(out, ((1, ), (2, 3)))'
...
project_dAR!(::Any, dAR::Union{ZeroTangent, NoTangent}, ::AbstractTensorMap, ::Symbol) = dAR
...
function ChainRulesCore.rrule(::typeof(vumps), T::MPOTensor; maxiter=250, tol=1e-12, kwargs...)
    ...
    function vumps_pushback_arnoldi(_∂ALAR)
        ...
        vals, vecs, info = realeigsolve(f_map, Y1, 1, :LM, Arnoldi(; tol=0.01*tol))
