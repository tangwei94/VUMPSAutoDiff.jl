...
@testset "Pulling through condition" for ix  in 1:5
    ...
    χ = 2
    d = 4
    Tmat = rand(Float64, d, d, d, d) # for d = 2, rotational symmetry will always guarantee the reflection symmetry
    Tmat = 0.5 * (Tmat + permutedims(Tmat, (2, 3, 4, 1)) + permutedims(Tmat, (3, 4, 1, 2)) + permutedims(Tmat, (4, 1, 2, 3)))
    Tmat = Tmat / norm(Tmat)
    @test norm(Tmat - permutedims(Tmat, (1, 4, 3, 2))) > sqrt(eps(1.0)) # does not have reflection symmetry. 
    T = TensorMap(permutedims(Tmat, (1, 2, 4, 3)), ℂ^d ⊗ ℂ^d, ℂ^d ⊗ ℂ^d)
        ...
        EL = rand(ComplexF64, VUMPSAutoDiff.left_space(TM));
        λ = VUMPSAutoDiff.left_env!(EL, TM)
        @test norm(left_transfer(TM, EL) - λ * EL) < sqrt(eps(1.0)) # TODO. add this to a specific test.

        @tensor B[-1 -2; -3] := EL[-1; -3 2] * δ[-2; 2]
        BR, L = right_canonical_QR(B)
        @test mps_fidelity(B, BR) > 1 - sqrt(eps(1.0)) # TODO. add this to a specific test.

        @tensor B1[-1 -2; -3] := B[1 -2; 2] * inv(L)[-1; 1] * L[2; -3]
        @test norm(B1[1]/BR[1] * BR - B1) < sqrt(eps(1.0)) # TODO. add this to a specific test

        # B -> BR, then A should follow the same gauge transformation as B
        @tensor A[-1 -2; -3] = AL[1 -2; 2] * inv(L)[-1; 1] * L[2; -3]

        @tensor Trot[-1 -2; -3 -4] := T[1 -1; -4 2] * δ[2 -2] * conj(δ[-3 1])
        @test norm(Trot - T) < sqrt(eps(1.0)) # TODO. wrap this as a test. C4 rotational invariance
        ...
        AL1, _ = left_canonical_QR(A)
        AL2, _ = left_canonical_QR(ER_B)
        U, meas = gauge_fixing(AL1, AL2)

        X1 
        X2


        @show mps_fidelity(A, ER_B) # A and ER_B are equivalent up to a gauge. 